...
@Component({
  ...
  template: `
    <brn-popover>
      <section hlmCard>
        <div hlmCardHeader>
          <h3 hlmCardTitle>Update your details</h3>
          <p hlmCardDescription>Insert new details below</p>
        ...
        <button brnPopoverTrigger type="submit" (click)="onSubmit()" hlmCardFooter hlmBtn variant="secondary" class="flex items-center text-md h-12 ml-5 mb-5">Update</button>

      </section>
      <div hlmPopoverContent class="w-80 ml-56 mt-1" *brnPopoverContent="let ctx">
        @if(gotInfo){
          <div class="space-y-2 mb-4">
              <h4 class="font-medium leading-none">Admin updated</h4>
...
export class UpdateAdminCardComponent {
  ...
  onSubmit() {
    this.adminService.updateAdmin({userName: this.username, passWord: this.password}, this.jwtToken).subscribe({
      next: (res) => {
        this.gotInfo = res;
